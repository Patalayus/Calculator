...
SET USER_VM_OPTIONS_FILE=%USERPROFILE%\.PyCharm50\pycharm%BITS%.exe.vmoptions
SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\pycharm%BITS%.exe.vmoptions
IF EXIST "%IDE_BIN_DIR%\win\pycharm%BITS%.exe.vmoptions" SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\pycharm%BITS%.exe.vmoptions
IF EXIST %USER_VM_OPTIONS_FILE% SET VM_OPTIONS_FILE=%USER_VM_OPTIONS_FILE%
IF NOT "%PYCHARM_VM_OPTIONS%" == "" SET VM_OPTIONS_FILE=%PYCHARM_VM_OPTIONS%
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_PYCHARM_%%p.log" "-Xbootclasspath/a:%IDE_HOME%/lib/boot.jar" -Didea.paths.selector=PyCharm50 %IDE_PROPERTIES_PROPERTY%
